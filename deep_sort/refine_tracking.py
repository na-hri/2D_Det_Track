...
#DATA_ROOT = '/data/datasets_home/datasets/Risk_Object_Identification/Clips'
DATA_ROOT = '/data/datasets_home/datasets'
TRACKING_PATH = osp.join(DATA_ROOT, 'retrieval_data_2d/tracking')
DETECTION_PATH = osp.join(DATA_ROOT, 'retrieval_data_2d/detection')
REFINED_PATH = osp.join(DATA_ROOT, 'retrieval_data_2d/tracking_refined')
#CAMERA_PATH = osp.join(DATA_ROOT, 'camera')
...
def max_IoU(ref_box, box_list):
  ...
  union_x2 = np.minimum(np.maximum(ref_box[0]+ref_box[2], box_list[:, 0]+box_list[:, 2]), 1280)
  union_y2 = np.minimum(np.maximum(ref_box[1]+ref_box[3], box_list[:, 1]+box_list[:, 3]), 720)
...
for sess, session in enumerate(SESSIONS):
  ...
  tracking_txt_path = osp.join(TRACKING_PATH, session, 'tracking.txt')

  tracking_results = np.loadtxt(tracking_txt_path, delimiter=',')

  detection_txt_path = osp.join(DETECTION_PATH, session, 'det_classes.txt')
  ...
  output_file = osp.join(REFINED_PATH, session, 'tracking_refined.txt')
