...
if __name__ == "__main__":
    ...
    for sequence in os.listdir(args.mot_dir):
        ...
        if not os.path.exists(sequence_dir):
            continue
        #result_file = os.path.join(args.result_dir, sequence, '%s_refined_iouthreshold.txt' % sequence)
        result_file = os.path.join(args.result_dir, sequence, 'tracking_refined.txt')
        update_ms = args.update_ms
        video_filename = os.path.join(args.output_dir, sequence, "%s_trackingoutput.avi" % sequence)

        #print("Saving %s to %s." % (sequence_txt, video_filename))
        print("Saving %s to %s." % (sequence, video_filename))
        show_results.run(
