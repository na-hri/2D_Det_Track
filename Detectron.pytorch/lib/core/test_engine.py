...
def test_net(
    ...
    for i, entry in enumerate(roidb):
        split = entry['image'].split('/')[6] 
        vid = entry['image'].split('/')[7] 
        imgid = entry['image'].split('/')[8] 
        roi_feat_path = '/data/datasets_home/datasets/Honda_Intersection_Dataset/features_grouped/roi_feat_resnetcoco/%s' % split
        if not os.path.isdir(roi_feat_path):
            os.system('mkdir %s' % roi_feat_path)
        #seg_path = '/data/datasets_home/datasets/Honda_Intersection_Dataset/frames_seg/%s/%s' % (split, vid)
        roi_feat_path = '/data/datasets_home/datasets/Honda_Intersection_Dataset/features_grouped/roi_feat_resnetcoco/%s/%s' % (split, vid)
        if not os.path.isdir(roi_feat_path):
            os.system('mkdir %s' % roi_feat_path)
        #seg_path = '/data/datasets_home/datasets/Honda_Intersection_Dataset/frames_seg/%s/%s/%s.png' % (split, vid, imgid[:-4])
        roi_feat_path = '/data/datasets_home/datasets/Honda_Intersection_Dataset/features_grouped/roi_feat_resnetcoco/%s/%s/%s_roi.npy' % (split, vid, imgid[:-4])
        if cfg.TEST.PRECOMPUTED_PROPOSALS:
            ...
            if len(box_proposals) == 0:
                roi_det_fts_perimage = np.empty((0,1024), dtype=np.float32)
                np.save(roi_feat_path, roi_det_fts_perimage) 
                continue
        else:
        ...
        cls_boxes_i, cls_segms_i, cls_keyps_i, cls_roi_det_fts_i = im_detect_all(model, im, box_proposals, timers)

        # Below script is for extracting proposal features only
        seg_path = None
        fid=1 
        print(roi_feat_path)
        check=checkin        
