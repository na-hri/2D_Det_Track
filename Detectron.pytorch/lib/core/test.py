...
def im_detect_all(model, im, box_proposals=None, timers=None):
    ...
    if cfg.TEST.BBOX_AUG.ENABLED:
        ...
        scores, boxes, im_scale, blob_conv, roi_det_fts = im_detect_bbox(
            model, im, cfg.TEST.SCALE, cfg.TEST.MAX_SIZE, box_proposals)
    timers['im_detect_bbox'].toc()
    assert roi_det_fts.shape[0] == boxes.shape[0]
    ...
    scores, boxes, cls_boxes, cls_roi_det_fts = box_results_with_nms_and_limit(scores, boxes, roi_det_fts)
    ...
    if cfg.MODEL.MASK_ON and boxes.shape[0] > 0:
        ...
        if cfg.TEST.MASK_AUG.ENABLED:
            ...
            masks, roi_seg_fts = im_detect_mask(model, im_scale, boxes, blob_conv)
        timers['im_detect_mask'].toc()

        timers['misc_mask'].tic()
        cls_segms, cls_roi_seg_fts = segm_results(cls_boxes, masks, boxes, roi_seg_fts, im.shape[0], im.shape[1])
    ...
    return cls_boxes, cls_segms, cls_keyps, cls_roi_det_fts, blob_conv
...
def im_detect_bbox(model, im, target_scale, target_max_size, boxes=None):
    ...
    return scores, pred_boxes, im_scale, return_dict['blob_conv'], return_dict['box_feat'].data.cpu().numpy()
...
def im_detect_mask(model, im_scale, boxes, blob_conv):
    ...
    pred_masks, roi_mask_feat = model.module.mask_net(blob_conv, inputs)
...
def box_results_with_nms_and_limit(scores, boxes, roi_det_fts):  # NOTE: support single-batch
    ...
    cls_roi_det_fts = [[] for _ in range(num_classes)]
    ...
    for j in range(1, num_classes):
        ...
        cls_roi_det_fts[j] = roi_det_fts[inds[keep],:]
    ...
    if cfg.TEST.DETECTIONS_PER_IM > 0:
        ...
        if len(image_scores) > cfg.TEST.DETECTIONS_PER_IM:
            ...
            for j in range(1, num_classes):
                ...
                cls_roi_det_fts[j] = cls_roi_det_fts[j][keep, :]
                assert cls_boxes[j].shape[0]==cls_roi_det_fts[j].shape[0]
    im_results = np.vstack([cls_boxes[j] for j in range(1, num_classes)])
    boxes = im_results[:, :-1]
    scores = im_results[:, -1]
    return scores, boxes, cls_boxes, cls_roi_det_fts
